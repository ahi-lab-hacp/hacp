...
<body><main>
  <header><div class="eyebrow">HACP 0.1 · Live reference demonstration</div><h1>Can this agent book the ticket?</h1><p>The receiver verifies the signed human intent, delegated mandate, authenticated agent, exact action, quantity, price limit, signature and replay nonce before creating a simulated booking.</p></header>
  ...
  <div class="actions"><button id="run" type="button">Run six verification cases</button><a class="link" href="/.well-known/hacp.json">Public keys</a></div>
  <section id="result" class="empty" aria-live="polite">Run the demonstration to compare an authorized request with wrong-identity, over-budget, intent-mismatch, tampered and replayed requests.</section>
